exports.init = function (dbname) {
    return [
        ...
        "CREATE TABLE " + dbname + ".datapoints (id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT, name TEXT);",
...
exports.insert = function (dbname, index, state, from, db) {
    ...
    return "INSERT INTO " + dbname + "." + db + " (id, ts, val, ack, _from, q) VALUES(" + index + ", " + state.ts + ", " + state.val + ", " + (state.ack ? 1 : 0) + ", " + (from || 0) + ", " + state.q + ");";
};
...
exports.getIdSelect = function (dbname, name) {
    return "SELECT id, type FROM " + dbname + ".datapoints WHERE name='" + name + "';";
};
