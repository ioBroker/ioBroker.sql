exports.init = function () {
    return [
        "CREATE TABLE sources (id INTEGER NOT NULL PRIMARY KEY, name TEXT);",
        "CREATE TABLE datapoints (id INTEGER NOT NULL PRIMARY KEY, name TEXT, type INTEGER);",
        "CREATE TABLE ts_number (id INTEGER NOT NULL, ts bigint, val REAL, ack boolean, _from INTEGER, ms INTEGER DEFAULT 0, q INTEGER, PRIMARY KEY(id, ts, ms));",
        "CREATE TABLE ts_string (id INTEGER NOT NULL, ts bigint, val TEXT, ack boolean, _from INTEGER, ms INTEGER DEFAULT 0, q INTEGER, PRIMARY KEY(id, ts, ms));"
    ];
};

exports.insert = function (index, state, from, db) {
    return "INSERT INTO " + db + "(id, ts, val, ack, _from, q, ms) VALUES(" + index + ", " + state.ts + ", " + state.val + ", " + (!!state.ack) + ", " + (from || 0)  + ", " + state.q + ", " + (state.ms || 0) + ");";
};
