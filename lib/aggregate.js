...
function beautify(options) {
    ...
    } else if (options.ignoreNull === 'false') { // include nulls
        options.ignoreNull = false;
    } else if (options.ignoreNull === '0') { // include nulls and replace them with 0
...
function sendResponse(adapter, msg, options, data, startTime) {
    ...
    if (data[0]) {
        ...
        if (!options.aggregate || options.aggregate === 'onchange' || options.aggregate === 'none' || options.alreadyAggregated) {
