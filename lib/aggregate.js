...
function sendResponse(adapter, msg, options, data, startTime) {
    ...
    if (options.count && !options.start && data.length > options.count) {
        // without start date we cut count elements from the end
        data.splice(0, data.length - options.count);
    }
    else if ((options.count) && (options.aggregate === 'none') && (data.length > options.count)) {
        // with a start date we cut count elements from the beginning
