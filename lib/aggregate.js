...
function aggregation(options, data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        preIndex = Math.round((data[i].ts - options.start) / options.step);
        // store all border values
        if (preIndex < 0) {
            index = 0;
        } else if (preIndex > options.maxIndex) {
            index = options.maxIndex + 1;
        } else {
            index = preIndex + 1;
        }
...
function beautify(options) {
    ...
    if (options.result.length) {
        ...
        if (firstTS > options.start) {
            if (preFirstValue && preFirstValue.ts !== firstTS) {
                // if steps
                if (options.aggregate === 'onchange' || options.aggregate === 'none' || !options.aggregate) {
        ...
        if (lastTS < options.end) {
            if (postLastValue && postLastValue.ts !== lastTS) {
                // if steps
                if (options.aggregate === 'onchange' || options.aggregate === 'none' || !options.aggregate) {
