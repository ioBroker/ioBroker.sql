...
exports.getDatabaseSize = function (dbname) {
    return `SELECT TABLE_SCHEMA AS 'database', SUM(data_length + index_length) / 1024 / 1024 / 1024 AS 'size' FROM information_schema.TABLES WHERE table_schema = '${dbname}'`;
}

exports.getTablesSize = function (dbname) {
    return `SELECT table_name AS 'name', ((data_length + index_length) / 1024 / 1024/ 1024) as 'size' FROM information_schema.TABLES WHERE table_schema = '${dbname}'`;
}