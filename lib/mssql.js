exports.init = function (dbname) {
    return [
        ...
        `CREATE TABLE ${dbname}.dbo.sources     (id INTEGER NOT NULL PRIMARY KEY IDENTITY(1,1), name constchar(255));`,
        `CREATE TABLE ${dbname}.dbo.datapoints  (id INTEGER NOT NULL PRIMARY KEY IDENTITY(1,1), name constchar(255), type INTEGER);`,
...
exports.getDatabaseSize = function (dbname) {
    return `SELECT TABLE_SCHEMA [database], ((sum((data_length + index_length)) / 1024) / 1024) [size] FROM information_schema.TABLES WHERE table_schema = '${dbname}'`;
}

exports.getTablesSize = function (dbname) {
    return `SELECT table_name [name], (((data_length + index_length) / 1024) / 1024) [size] FROM information_schema.TABLES WHERE table_schema = '${dbname}'`;
}
...
exports.getDataSetsFromTable = function (dbname, tableName) {
    return `SELECT id, Count(id) [count], IIF(id NOT IN (SELECT id from ${dbname}.datapoints), 1, 0) [dead] FROM ${dbname}.${tableName} GROUP BY id`;
}