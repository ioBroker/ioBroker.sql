...
class SqlAdapter extends adapter_core_1.Adapter {
     ...
     */
    logConnectionError(err, prefix) {
        const text = `${prefix ? `${prefix}: ` : ''}${(0, errors_1.formatError)(err)}`;
        const now = Date.now();
        ...
        try {
            ...
            const err = await new Promise(resolve => client.connect((err) => {
                if (err) {
                    ...
                    this.sendTo(msg.from, msg.command, { error: (0, errors_1.formatError)(err) }, msg.callback);
                    return;
                }
            ...
            if (this.testConnectTimeout) {
                clearTimeout(this.testConnectTimeout);
                this.testConnectTimeout = null;
                this.sendTo(msg.from, msg.command, { error: err ? (0, errors_1.formatError)(err) : null }, msg.callback);
                return;
            }
        }
        catch (ex) {
    ...
    _userQuery(msg, callback) {
        try {
            ...
            this.borrowClientFromPool((err, client) => {
                if (err || !client) {
                    this.sendTo(msg.from, msg.command, { error: err ? (0, errors_1.formatError)(err) : 'No client' }, msg.callback);
    ...
    _checkRetention(query, cb) {
        ...
        this.borrowClientFromPool((err, client) => {
            if (err || !client) {
                this.returnClientToPool(client);
                this.log.error(err ? (0, errors_1.formatError)(err) : 'No client');
                cb?.();
            }
            else {
    ...
    #updateInDb(id, table, index, state, cb) {
    ...
    #deleteFromDb(id, table, index, state, cb) {
    ...
    #buildDeleteQuery(table, index, state) {
        if (state.start && state.end) {
    ...
    updateState(msg) {
        ...
        if (Array.isArray(msg.message)) {
            ...
            id = this.aliasMap[msg.message.id] ? this.aliasMap[msg.message.id] : msg.message.id;
            return this.update(id, msg.message.state, err => this.sendTo(msg.from, msg.command, err
                ? { error: err.message, sqlConnected: !!this.clientPool }
                : { success: true, sqlConnected: !!this.clientPool }, msg.callback));
        }
        else {
            this.log.error('updateState called with invalid data');
            return this.sendTo(msg.from, msg.command, {
    ...
    deleteHistoryEntry(msg) {
        ...
        if (Array.isArray(msg.message)) {
            ...
            id = this.aliasMap[msg.message.id] ? this.aliasMap[msg.message.id] : msg.message.id;
            return this.#delete(id, { ts: msg.message.state.ts }, err => this.sendTo(msg.from, msg.command, err
                ? { error: err.message, sqlConnected: !!this.clientPool }
                : { success: true, sqlConnected: !!this.clientPool }, msg.callback));
        }
            ...
            this.log.debug('deleteHistoryEntry 1 item');
            id = this.aliasMap[msg.message.id] ? this.aliasMap[msg.message.id] : msg.message.id;
            return this.#delete(id, { ts: msg.message.ts }, err => this.sendTo(msg.from, msg.command, err
                ? { error: err.message, sqlConnected: !!this.clientPool }
                : { success: true, sqlConnected: !!this.clientPool }, msg.callback));
        }
        else {
    ...
    deleteStateAll(msg) {
        ...
        if (Array.isArray(msg.message)) {
            ...
            id = this.aliasMap[msg.message.id] ? this.aliasMap[msg.message.id] : msg.message.id;
            return this.#delete(id, {}, err => this.sendTo(msg.from, msg.command, err
                ? { error: err.message, sqlConnected: !!this.clientPool }
                : { success: true, sqlConnected: !!this.clientPool }, msg.callback));
        }
        else {
    ...
    getRawEntries(msg) {
        ...
        this.#readIdIndexAndType(id, (err, index, type) => {
            if (err || index === undefined || type === undefined) {
                return this.sendTo(msg.from, msg.command, { error: (err || new Error(`Unknown ID: ${id}`)).message }, msg.callback);
            }
            ...
            this.borrowClientFromPool((err, client) => {
                ...
                client.execute(countQuery, (err, rows) => {
                    if (err) {
                        this.returnClientToPool(client);
                    ...
                    const total = rows?.[0] ? parseInt(rows[0].total, 10) || 0 : 0;
