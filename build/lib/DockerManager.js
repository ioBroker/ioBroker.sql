...
class DockerManager {
    ...
    constructor(adapter, options, containers) {
        ...
        this.#waitAllChecked = new Promise(resolve => (this.#waitAllCheckedResolve = resolve));
    }
    ...
    static mapInspectToConfig(inspect) {
        const obj = {
            ...
            security: {
                ...
                apparmor: inspect.HostConfig.SecurityOpt?.find(opt => opt.startsWith('apparmor='))?.split('=')[1] ??
                    undefined,
    ...
    async #init() {
        ...
        else if (await _a.checkDockerSocket()) {
            this.#driver = 'socket';
            this.#dockerode = new dockerode_1.default({ socketPath: '/var/run/docker.sock' });
        }
        else if (await _a.isDockerApiRunningOnPort(2375)) {
            this.#driver = 'http';
            this.#dockerode = new dockerode_1.default({ protocol: 'http', host: '127.0.0.1', port: 2375 });
        }
        else if (await _a.isDockerApiRunningOnPort(2376)) {
            this.#driver = 'http';
            this.#dockerode = new dockerode_1.default({ protocol: 'http', host: '127.0.0.1', port: 2376 });
        }
        else {
            this.#driver = 'cli';
                ...
                try {
                    ...
                    if (container.networkMode &&
                        ...
                        if (container.networkMode !== prefix && !container.networkMode.startsWith(`${prefix}_`)) {
                            this.adapter.log.debug(`Renaming network ${container.networkMode} to be prefixed with ${prefix}_`);
                        ...
                        for (const mount of container.mounts) {
                            if (mount.type === 'volume' && mount.source) {
                                ...
                                if (mount.source !== prefix && !mount.source.startsWith(`${prefix}_`)) {
                                    this.adapter.log.debug(`Renaming volume ${mount.source} to be prefixed with ${prefix}_`);
                                ...
                                if (mount.iobBackup) {
                                    if (!container.labels.iob_backup) {
                                        ...
                                        const volumes = container.labels.iob_backup
                                            .split(',')
                                            .map(v => v.trim())
                                            .filter(v => v);
    ...
    static getDockerodeConfig(config) {
        ...
        return {
            ...
            // { '/data': {} }
            Volumes: config.volumes?.reduce((acc, vol) => (acc[vol] = {}), {}),
    ...
    async containerRun(config) {
        if (this.#dockerode) {
            const container = await this.#dockerode.createContainer(_a.getDockerodeConfig(config));
            await container.start();
            return { stdout: `Container ${config.name} started`, stderr: '' };
        }
        try {
            return await this.#exec(`run ${_a.toDockerRun(config)}`);
        }
        catch (e) {
            return { stdout: '', stderr: e.message.toString() };
        }
    ...
    async containerCreate(config) {
        ...
        try {
            return await this.#exec(`create ${_a.toDockerRun(config, true)}`);
        }
        catch (e) {
            return { stdout: '', stderr: e.message.toString() };
        }
    ...
    async containerReCreate(config) {
        if (this.#dockerode) {
            ...
            const newContainer = await this.#dockerode.createContainer(_a.getDockerodeConfig(config));
    ...
    static rodeInspect2DockerImageInspect(data) {
        return {
    ...
    async imageInspect(imageId) {
        if (this.#dockerode) {
            ...
            return _a.rodeInspect2DockerImageInspect(data);
        }
        try {
    ...
    async getIpOfContainer(containerName) {
    ...
    async containerList(all = true) {
        if (this.#dockerode) {
            ...
            return containers.map(cont => {
                const statusKey = cont.State.toLowerCase();
                let status;
                if (statusKey === 'up') {
                    status = 'running';
                }
                ...
                return {
                    ...
                    ports: cont.Ports.map(p => `${p.IP ? `${p.IP}:` : ''}${p.PublicPort ? `${p.PublicPort}->` : ''}${p.PrivatePort}/${p.Type}`).join(', '),
        ...
        try {
            const { stdout } = await this.#exec(`ps ${all ? '-a' : ''} --format  "{{.Names}};{{.Status}};{{.ID}};{{.Image}};{{.Command}};{{.CreatedAt}};{{.Ports}};{{.Labels}}"`);
            return stdout
                .split('\n')
                .filter(line => line.trim() !== '')
                .map(line => {
                ...
                return {
                    ...
                    ports,
                    names,
                    labels: labels?.split(',').reduce((acc, label) => {
    ...
    static dockerodeInspect2DockerContainerInspect(data) {
        return {
            ...data,
        };
    }
    ...
    async containerInspect(containerNameOrId) {
        if (this.#dockerode) {
            try {
                ...
                const result = _a.dockerodeInspect2DockerContainerInspect(dResult);
     ...
     */
    static toDockerRun(config, create) {
        const args = [];
        // detach / interactive
    ...
    async volumeList() {
        ...
        try {
            ...
            return stdout
                ...
                return { name, driver: driver, volume };
            });
        }
        catch (e) {
            this.adapter.log.debug(`Cannot list networks: ${e.message.toString()}`);
    ...
    async volumeCopyTo(volumeName, sourcePath) {
        ...
        // create a temporary container with volume mounted
        const createResult = await this.#exec(`create -v ${volumeName}:/data --name ${tempContainerName} alpine sleep 60`);
     ...
     */
    async volumeCreate(name, driver, volume) {
        if (this.#dockerode) {
        ...
        if (driver === 'local' || !driver) {
            if (volume) {
                result = await this.#exec(`volume create local --opt type=none --opt device=${volume} --opt o=bind ${name}`);
            }
            else {
