...
class SQLClientPool {
    static MESSAGES = {
        ...
        MAX_WAIT: 'The maximum number of clients are already active and the maximum wait time has been exceeded; cannot obtain a new client.',
    ...
    sqlOptions;
    factory;
    constructor(poolOptions, sqlOptions, factory) {
    ...
    close(callback) {
        ...
        if (this.active > 0) {
            callback?.(new Error(_a.MESSAGES.CLOSED_WITH_ACTIVE));
            return;
        }
        callback?.();
    }
    execute(sql, callback) {
        this.borrow((err, client) => {
            ...
            client.execute(sql, (err, result) => this.return(client, () => callback(err, result)));
        });
    }
    borrow(callback, blockedSince = null, retryCount = 0) {
        if (typeof callback !== 'function') {
            throw new Error(_a.MESSAGES.INVALID_ARGUMENT);
        }
        if (!this.poolIsOpen) {
            return callback(new Error(_a.MESSAGES.POOL_NOT_OPEN));
        }
        if (this.poolOptions.max_active &&
            this.active >= this.poolOptions.max_active &&
            this.poolOptions.when_exhausted === 'fail') {
            return callback(new Error(_a.MESSAGES.EXHAUSTED));
        }
        if (this.poolOptions.max_active &&
            this.active >= this.poolOptions.max_active &&
            this.poolOptions.when_exhausted === 'block') {
            if (blockedSince && Date.now() - blockedSince >= this.poolOptions.max_wait) {
                return callback(new Error(_a.MESSAGES.MAX_WAIT));
            }
        ...
        return this.create((err, client) => {
            ...
            return this.#activateAndValidateOrDestroy(client, (err, valid, client) => {
                ...
                if (!valid || !client) {
                    ...
                    return callback(new Error(_a.MESSAGES.INVALID));
                }
                client.pooled_at = null;
                client.borrowed_at = Date.now();
                this.active++;
    ...
    return(client, callback) {
        if (!client) {
            callback?.(new Error(_a.MESSAGES.NULL_RETURNED));
            return;
        }
        if (this.active <= 0) {
            callback?.(new Error(_a.MESSAGES.TOO_MANY_RETURNED));
            return;
        }
        this.returned++;
        this.active--;
        this.passivate(client, (err, client) => {
            if (err || !client) {
                callback?.(err || new Error(_a.MESSAGES.NULL_RETURNED));
                return;
            }
    ...
    _config(opts, callback) {
        ...
        if (opts.max_retries) {
            if (opts.retry_interval && (typeof opts.retry_interval !== 'number' || opts.retry_interval <= 0)) {
                ...
                opts.retry_interval = _a.DEFAULT_RETRY_INTERVAL;
            }
        }
        ...
        if (typeof opts.wait_interval !== 'number' || opts.wait_interval < 0) {
            opts.wait_interval = _a.DEFAULT_WAIT_INTERVAL;
        }
    ...
    #reconfig(callback) {
        ...
        this._evict(err => {
            ...
            this._prepopulate((err, borrowed) => {
    ...
    #evictionRun(numToCheck, callback) {
        const newPool = [];
        let numChecked = 0;
    ...
    #returnN(borrowed, callback) {
        if (!Array.isArray(borrowed)) {
            callback(new Error(_a.MESSAGES.INTERNAL_ERROR));
            return;
        }
    ...
    #activateAndValidateOrDestroy(client, callback) {
...
_a = SQLClientPool;
//# sourceMappingURL=sql-client-pool.js.map